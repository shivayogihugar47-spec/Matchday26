...
const stadiumVenues = [
  {
    id: 1, name: 'Fuku Chicken',
    description: 'Spicy chicken sandwich, crispy fingers, and loaded fries',
    location: 'Sections 117, 224, 330', image: '/fuku.jpg',
    icon: <Utensils className="w-5 h-5" />, accent: 'from-orange-500 to-red-600',
    glow: 'rgba(249,115,22,0.55)', tag: 'Fan Favourite', featured: true,
  ...
  {
    id: 2, name: "Nonna Fusco's Kitchen",
    description: 'Authentic meatball subs, pasta dishes, and fresh zeppoles',
    location: 'Sections 118, 215, 239', image: "/Nonna Fusco's Kitchen.jpg",
    icon: <Utensils className="w-4 h-4" />, accent: 'from-rose-500 to-pink-600',
    glow: 'rgba(244,63,94,0.5)', tag: 'Italian',
  ...
  {
    id: 3, name: "Shah's Halal",
    description: 'Gyro platters, chicken over rice, and pakora chips',
    location: 'Sections 126, 314, MetLife West Hall', image: "/Shah's Halal.jpg",
    icon: <Truck className="w-4 h-4" />, accent: 'from-emerald-500 to-teal-600',
    glow: 'rgba(16,185,129,0.5)', tag: 'Halal',
  ...
  {
    id: 4, name: 'Mrs. Fields',
    description: 'Warm chocolate chip cookies, brownies, and hot cocoa',
    location: 'All concourses', image: '/Mrs. Fields.jpg',
    icon: <Coffee className="w-4 h-4" />, accent: 'from-amber-500 to-yellow-600',
    glow: 'rgba(245,158,11,0.5)', tag: 'Desserts',
  ...
  {
    id: 5, name: 'Boardwalk Fryer',
    description: 'Crispy chicken tenders, fried clams, and classic hot dogs',
    location: 'Sections 135, 143, 217, 330', image: '/Boardwalk Fryer.jpg',
    icon: <Utensils className="w-4 h-4" />, accent: 'from-sky-500 to-blue-600',
    glow: 'rgba(14,165,233,0.5)', tag: 'American',
    contact: '(201) 559-1504',
    menu: [
      { name: 'Classic Hot Dog',      price: '$12', description: 'Grilled, ketchup/mustard' },
  ...
  {
    id: 6, name: "Patty's Burger",
    description: 'Signature juicy burgers, cheese fries, and cold drinks',
    location: 'Sections 106, 131, 201, 313, 346', image: "/Patty's Burger.png",
    icon: <Utensils className="w-4 h-4" />, accent: 'from-orange-400 to-amber-600',
    glow: 'rgba(251,146,60,0.5)', tag: 'Burgers',
...
function FeaturedCard({ venue, onClick }) {
  return (
    <article
      onClick={onClick}
      className="group relative cursor-pointer overflow-hidden rounded-[1.75rem] transition-shadow duration-300 hover:shadow-[0_0_0_1.5px_rgba(249,115,22,0.5),0_24px_60px_-12px_rgba(249,115,22,0.3)]"
      style={{ height: '240px' }}
    >
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className={`absolute -right-8 -top-8 h-32 w-32 rotate-45 bg-gradient-to-br ${venue.accent} opacity-60 blur-xl`} />
      </div>
      <div className="relative z-10 flex h-full flex-col justify-between p-5">
        <div className="flex items-start justify-between">
          ...
          <div className={`flex items-center gap-1 rounded-full bg-gradient-to-r ${venue.accent} px-2.5 py-1.5`}>
            <Star className="h-3 w-3 fill-white text-white" />
            <span className="text-[9px] font-black uppercase tracking-widest text-white">Top Pick</span>
          </div>
        </div>
        <div>
          <h4 className="font-display text-2xl font-black leading-tight text-white">{venue.name}</h4>
          <div className="mt-2 flex items-center gap-2">
            <div className={`h-1.5 w-1.5 rounded-full bg-gradient-to-br ${venue.accent}`} />
            <span className="text-xs font-semibold text-slate-300">{venue.location}</span>
          </div>
        </div>
      </div>
    </article>
...
/* ── Small card ── */
function VenueCard({ venue, index, cardHeight = 160, onClick }) {
  ...
  return (
    <article
      ...
      className="group relative cursor-pointer overflow-hidden rounded-[1.75rem] transition-shadow duration-300"
      style={{
        height: `${cardHeight}px`,
        boxShadow: hovered
          ? `0 0 0 1.5px ${venue.glow}, 0 20px 50px -10px ${venue.glow}`
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute -right-2 -top-3 select-none pointer-events-none font-black text-[5.5rem] leading-none text-white/[0.05]">
          {String(index + 1).padStart(2, '0')}
        </div>

        {/* Tag pill — CSS opacity transition */}
        <div className={`absolute left-3 top-3 rounded-full bg-gradient-to-r ${venue.accent} px-2.5 py-1 shadow-lg transition-all duration-200 ${hovered ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'}`}>
          <span className="text-[9px] font-black uppercase tracking-[0.28em] text-white">{venue.tag}</span>
        </div>

        <div className="absolute inset-x-0 bottom-0 p-4">
          <div className="flex items-center gap-2.5">
            <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${venue.accent} text-white shadow-lg transition-transform duration-200 ${hovered ? 'scale-110' : 'scale-100'}`}>
              {venue.icon}
            </div>
            <h4 className="font-display text-[15px] font-bold leading-tight text-white">{venue.name}</h4>
          </div>

          {/* Info reveal — CSS max-height transition */}
          <div className={`overflow-hidden transition-all duration-250 ${hovered ? 'max-h-20 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
            <p className="text-[11px] leading-relaxed text-white/75 line-clamp-2">{venue.description}</p>
            <div className="mt-1 flex items-center gap-1.5">
              <MapPin className="h-3 w-3 flex-shrink-0 text-slate-400" />
              <span className="truncate text-[10px] font-semibold text-slate-300">{venue.location}</span>
            </div>
          </div>
        </div>
      </div>
...
/* ── Menu Modal — CSS transition via dialog backdrop ── */
  ...
  return (
    ...
    >
      <div
        ...
        className="my-8 flex w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-white/10 bg-slate-900/95 shadow-2xl"
      >
        {/* Hero image */}
        <div className="relative h-48 flex-shrink-0 overflow-hidden">
          ...
          <button
            onClick={onClose}
            className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full bg-slate-950/80 text-white transition-colors hover:bg-slate-900"
          >
            <X className="h-4 w-4" />
          </button>
          <div className="absolute bottom-4 left-4">
            <h3 className="font-display text-2xl font-black text-white">{venue.name}</h3>
            <p className="text-sm text-slate-300">{venue.location}</p>
          </div>
        </div>

        {/* Contact */}
        <div className="mx-6 mt-5 flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <a
            ...
            className={`flex items-center gap-2 rounded-xl bg-gradient-to-r ${venue.accent} px-4 py-2 font-bold text-white transition-opacity hover:opacity-90`}
          >
        ...
        <div className="mx-6 my-5">
          ...
          <div className="space-y-3">
            {venue.menu.map((item) => (
              <div key={item.name} className="flex items-start justify-between rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="relative z-10">
        ...
        <div className="mb-7 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-center gap-3.5">
            <div className="flex h-[3.25rem] w-[3.25rem] items-center justify-center rounded-[1.3rem] bg-gradient-to-br from-orange-400 via-amber-500 to-yellow-500 shadow-[0_12px_32px_rgba(251,191,36,0.32)]">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-slate-400">Best concessions and amenities around the bowl</p>
            </div>
          </div>
