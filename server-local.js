...
app.get('/api/concierge/live-status', (req, res) => {
...
- Accessibility Mode: ${accessibility === 'true' ? "Enabled" : "Disabled"}
  ...
  res.json({
    data: {
      ...
      variableValues: {
        ...
        accessibilityMode: accessibility === 'true' ? "Enabled" : "Disabled",
...
app.post('/api/exit-plan/generate', (req, res) => {
  const { transportMethod, hasAccessibleNeeds, preferredGate } = req.body;
...
app.all('/api/lost-and-found/check', (req, res) => {
  let params = req.method === 'GET' ? req.query : (req.body.parameters || req.body);
